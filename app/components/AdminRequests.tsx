...
const Table = ({ EnrollmentRequests }: any) => {
  const [filteredEnrollments, setFilteredEnrollments] =
    useState(EnrollmentRequests);
  ...
  const handleStateChange = async (id: string, status: string) => {
    ...
    try {
      ...
      const updatedEnrollments = EnrollmentRequests.map((enrollment: any) =>
  ...
  useEffect(() => {
    let filteredData = EnrollmentRequests;
  ...
  }, [searchVIN, searchModel, searchMake, selectedStatus, EnrollmentRequests]);
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <table className="min-w-full">
        <thead>
          <tr>
            <th className="px-6 py-3 bg-gray-100 text-left text-xs leading-4 font-medium text-gray-800 uppercase tracking-wider">
              Name
            </th>
            <th className="px-6 py-3 bg-gray-100 text-left text-xs leading-4 font-medium text-gray-800 uppercase tracking-wider">
              Status
